...
class ResolutionSender {
    ...
    void run(issue) {
        ...
        def assignee_resol = ComponentAccessor.getCustomFieldManager().getCustomFieldObject("customfield_11300")
        if (issue.getResolution().getName() == "Передано в сервисный центр" &&
                assignee_resol.getValue(issue).toString() == issue.getResolution().getName()) {
            dl.debug("to service")
...
try {
    ...
    def script_name = "MigrationToDepartment_CommentByResolution" + this.class.getName()
